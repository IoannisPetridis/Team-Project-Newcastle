...
int main()
{
	...
	AssetsManager::Player_1->SetName(MainWindow::GetPlayerName());
	GameTimer engine_timer;

	Client *client = new Client(scene);
	if (client->connection()) {
		std::thread t(&Client::initial_run, client);
		t.join();
		t.~thread();
	}
	...
	while (Window::GetWindow().UpdateWindow() && !Window::GetKeyboard()->KeyDown(KEYBOARD_ESCAPE)){
		...
		if (!PhysicsEngine::Instance()->IsGameover()) {
			...
			std::thread t1(&Client::run, client);
			t1.join();
			t1.~thread();
			...
			std::thread t2(&Client::run, client);
			t2.join();
			t2.~thread();
			...
			std::cout << AssetsManager::Player_1->GetName() << std::endl; 
