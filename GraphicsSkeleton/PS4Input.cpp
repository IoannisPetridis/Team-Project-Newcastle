...
void PS4Input::Poll() {
	...
	if (ret == SCE_OK) {
		if (data.connected) {
			...
			buttons[10] = ((data.buttons & SCE_PAD_BUTTON_UP) ? 1.0f : 0.0f);
			buttons[11] = ((data.buttons & SCE_PAD_BUTTON_RIGHT) ? 1.0f : 0.0f);
			buttons[12] = ((data.buttons & SCE_PAD_BUTTON_DOWN) ? 1.0f : 0.0f);
			buttons[13] = ((data.buttons & SCE_PAD_BUTTON_LEFT) ? 1.0f : 0.0f);
		}
	}
}