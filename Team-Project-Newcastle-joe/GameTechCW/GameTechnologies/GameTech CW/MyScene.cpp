...
bool MyScene::InitialiseGL()
{
	...
	////FMOD
	/*audio.CreateSystem(result, system2);
	
	audio.LoadSound("drumloop.wav", sound1, result, system2);

	FMOD_VECTOR pos = { m_Sphere->Physics()->GetPosition().x, m_Sphere->Physics()->GetPosition().y, m_Sphere->Physics()->GetPosition().z };
	FMOD_VECTOR vel = { 0.0f, 0.0f, 0.0f };
	audio.AddSound(pos, vel , result, system2, channel1,sound1);
...
//	audio->GetCameraInfo(m_Camera , result ,system2 , CameraPosition, CameraForward , CameraUp , CameraVelocity , LastPosition);
	...
	if (!Window::GetKeyboard()->KeyHeld(KEYBOARD_Q) 
		...
		this->AddGameObject(throw_S);
		//result = system2->playSound(sound2, 0, false, &channel2);
