...
bool CollisionDetection::CheckSphereSphereCollision(const PhysicsObject* obj1, const PhysicsObject* obj2, const CollisionShape* shape1, const CollisionShape* shape2, CollisionData* out_coldata) const
{
	...
	*/
	//check that both sphere are  sphere
	...
	//True if the distance between centre points is less or eqal to the sum of the two radia
}
...
void AddPossibleCollisionAxis(Vector3& axis, std::vector<Vector3>* possible_collision_axes)
{
	...
	for (const Vector3& p_axis : *possible_collision_axes)
	{
		...
		if (Vector3::Dot(axis, p_axis) < epsilon)
			return;
	}
...
bool CollisionDetection::CheckCollision(const PhysicsObject* obj1, const PhysicsObject* obj2, const CollisionShape* shape1, const CollisionShape* shape2, CollisionData* out_coldata) const
{
	...
	*/
	...
	//Now we need to check the edge-edege cases
	...
	//now we need to handel the specific case of spheres, which have infinite collsion axes and edges
	bool shape1_isSphere = shape1_edges.empty();
	bool shape2_isSphere = shape2_edges.empty();

	//if both are spheres ,then the only axes we have to check is between the two centre points
	...
	//For all possible axes, check each one unit a seperating
	//axies between the two objects can be found - such proving
	//the object cannot be colliding
	for (const Vector3& axis : possible_collision_axes)
	{
		//if the collision axis does not intersect then return
		//immediatrly as we know that atleast in one direction/axis
		//the two object s does not intersect
	...
	//NCLDebug::DrawHairLine(obj2->GetPosition(),obj2->GetPosition()+best_colData.normal * best_colData.penetration,Vector4(1.0f,1.0f,1.0f,1.0f));
...
bool CollisionDetection::CheckCollisionAxis(const Vector3& axis, const PhysicsObject* obj1, const PhysicsObject* obj2, const CollisionShape* shape1, const CollisionShape* shape2, CollisionData* out_coldata) const
{
	...
	*/
	...
	//get the max and min for the vertices along the axis from the shape1 and shape2
...
bool CollisionDetection::BuildCollisionManifold(const PhysicsObject* obj1, const PhysicsObject* obj2, const CollisionShape* shape1, const CollisionShape* shape2, const CollisionData& coldata, Manifold* manifold) const
{
	...
	*/
	...
	//get the require face infomation for the two shapes around the collision normal
	...
	//if either shape1 or shape2 returned a single point, then it must be on a curve and thus the only contact point to generate is already available

	if (polygon1.size() == 0 || polygon2.size() == 0)
	{
		//no point returned, resulting in on possible contact point
		return false;
	}
	...
	{
		//other wise use clipping to cut down the incident face to fit inside the reference planes using the surrounding face planes
		...
		Plane refPlane;
		//get the incident and the reference polygons
		...
		//Clip the incident face into the adjasent edges of the reference face
		...
		for (const Vector3& endPoint : *incPolygon)
		{
			...
			if (flipped)
			{
				...
				//contact_penrtration = min(contact_penetration,0.0f);
			...
			{
				//Calculation distance to ref plane/face
