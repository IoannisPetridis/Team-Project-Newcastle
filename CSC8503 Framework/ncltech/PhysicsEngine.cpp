		...
		for (CollisionPair& cp : m_BroadphaseCollisionPairs)
		{
			...
			if (CollisionDetection::Instance()->CheckCollision(cp.objectA, cp.objectB, shapeA, shapeB, &coldata))//if it collision
			{
				...
				if (okA && okB)
				{
					...
					if((!cp.objectA->GetLinearVelocity().LengthSquared() < 0.001f && !cp.objectA->GetAngularVelocity().LengthSquared() < 0.001f) 
					|| (!cp.objectB->GetLinearVelocity().LengthSquared() < 0.001f && !cp.objectB->GetAngularVelocity().LengthSquared() < 0.001f))
