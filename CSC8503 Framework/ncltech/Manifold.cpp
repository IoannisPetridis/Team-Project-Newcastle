...
void Manifold::ApplyImpulse()
{
	...
	*/
	...
	for (Contact & contact : m_Contacts)
	{
		...
		// float friction_limit = sqrtf (2.0 f) * contact . normal . impulseSum ;
...
void Manifold::UpdateConstraint(Contact& contact)
{
	...
	*/
	...
	// Elasticity
	{
		float elasticity = (m_NodeA->GetElasticity() + m_NodeB->GetElasticity()) * 0.5f;
	...
	float elatisity_term = elasticity * Vector3::Dot(contact.collisionNormal,
		...
		contact.normal = Constraint(m_NodeA, m_NodeB, - contact.collisionNormal,
		Vector3::Cross(-r1, contact.collisionNormal),contact.collisionNormal, Vector3::Cross(r2, contact.collisionNormal),b);
