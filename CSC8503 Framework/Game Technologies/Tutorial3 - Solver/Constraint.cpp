...
Constraint::Constraint(PhysicsObject* objA, PhysicsObject* objB, Vector3 j1, Vector3 j2, Vector3 j3, Vector3 j4, float b)
{
	this->objA = objA;
	this->objB = objB;
...
void Constraint::ApplyImpulse()
{
	...
	//J * M(-1)*J(t)
	float contstraint_mass =objA->GetInverseMass()* Vector3::Dot(j1, j1) + Vector3::Dot(j2, (objA->GetInverseInertia()*j2)) +
		objB->GetInverseMass() * Vector3::Dot(j3, j3) +	Vector3::Dot(j4, (objB->GetInverseInertia()*j4)) +
		softness;
