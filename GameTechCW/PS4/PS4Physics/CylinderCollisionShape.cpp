...
CylinderCollisionShape::CylinderCollisionShape(const float length, const float radius, GLMVector3 halfdim) :
m_Radius(radius), m_length(length), m_CuboidHalfDimensions(halfdim)
{
	m_Invprecision = 30.0f;
	m_CuboidHalfDimensions = GLMVector3(1.0f , 1.0f, 1.0f);
...
void CylinderCollisionShape::ConstructCubeHull() {
	...
	for (float i = 0; i <= 2 * PI; i += angle) {
		...
		m_CubeHull.AddVertex(GLMVector3(first_x, halflen, first_z));
		list3_Top[j] = counter;
		cyside->list4[0] = counter;
		counter++;

		m_CubeHull.AddVertex(GLMVector3(last_x, halflen, last_z));
		list3_Top[++j] = counter;
		...
		cyside->normal = GLMVector3::Cross((GLMVector3(first_x, halflen, first_z) - GLMVector3(last_x, halflen, last_z)),
			(GLMVector3(first_x, -halflen, first_z) - GLMVector3(first_x, halflen, first_z)));

		m_CubeHull.AddFace(cyside->normal, 4, cyside->list4);
	}

	m_CubeHull.AddFace(GLMVector3(0.0f, 1.0f, 0.0f), static_cast<int>(360.0f / m_Invprecision), list3_Top);
	m_CubeHull.AddFace(GLMVector3(0.0f, -1.0f, 0.0f), static_cast<int>(360.0f / m_Invprecision), list3_Bottom);
}


GLMMatrix3 CylinderCollisionShape::BuildInverseInertia(float invMass) const
{
	GLMMatrix3 inertia;
...
void CylinderCollisionShape::GetCollisionAxes(const PhysicsObject* currentObject, std::vector<GLMVector3>* out_axes) const
{
	if (out_axes)
	{
...
void CylinderCollisionShape::GetMinMaxVertexOnAxis(const PhysicsObject* currentObject, const GLMVector3& axis, GLMVector3* out_min, GLMVector3* out_max) const
{
...
void CylinderCollisionShape::GetIncidentReferencePolygon(const PhysicsObject* currentObject, const GLMVector3& axis, std::list<GLMVector3>* out_face, GLMVector3* out_normal, std::vector<Plane>* out_adjacent_planes) const
{
	GLMMatrix4 wsTransform = currentObject->GetWorldSpaceTransform() * GLMMatrix4::Scale(m_CuboidHalfDimensions);
