...
PS4RendererBase::PS4RendererBase()
	...
	_MaxCMDBufferCount(3),
	  _bufferCount(3),	//how many screen buffer we want to have
	  _GarlicMemory(1024 * 1024 * 512),
	  _OnionMemory( 1024 * 1024 * 256)
{

	//multiple screen buffer
	framesSubmitted		 = 0;
	currentGPUBuffer	 = 0;
	...
	//stack memory on stack garlic/onion
	InitialiseMemoryAllocators();

	//initialise space for frames, call the gnm toolkit initialization function for
	//garlic and onion memory allocation
	InitialiseGCMRendering();

	//generate Screen buffers 
...
void	PS4RendererBase::InitialiseMemoryAllocators() {
	...
	//pass interface from stackallocator class to local allocator variable
...
void	PS4RendererBase::DestroyGCMRendering() {
	//PS4MemoryAware::onionAllocator.release(frames);
}
...
void PS4RendererBase::RenderScene()			{
	...
	DrawMesh(*defaultMesh_2);

	currentFrame->EndFrame();

	framesSubmitted++;

	cout << framesSubmitted << endl;
}
...
void PS4RendererBase::DrawMesh(PS4Mesh& mesh) {	
	...
	modelMatrix = Matrix4::rotationZ(rad);
