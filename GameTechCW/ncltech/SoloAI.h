...
class SoloBaseState; //foreward declaration of base state so compiler is awware of base states existance

class SoloAI : public SimpleMeshObject { //ArbitraryAI object inherits from simplemeshobject and holds information of AI's hitbox and physics
public:
	SoloAI(const std::string& name, Scene* m_scene); //on instantiation must pass its constructor name string to be given to simplemesh object (as its constructor requires it), creates string of inheritance GameObject->SimpleMeshObject->ArbitraryAI || also pass the instance of the camera to the AI so that it can access things like GetPosition
	SoloAI();
	~SoloAI();
	...
	Scene* scene;
	bool left, right, reverse, forward, jump, statebool;
	Vector3 Node, DirectionVector;

private:
	SoloBaseState* currentState; //pointer to the current state of the state machine, ie u can use currentState in methods within ArbitraryAI to do broadly the same thing, while the implementation changes depending on which state currentState is actually pointing too
	int iterator, rightiterator, leftiterator, jumpiterator;
};