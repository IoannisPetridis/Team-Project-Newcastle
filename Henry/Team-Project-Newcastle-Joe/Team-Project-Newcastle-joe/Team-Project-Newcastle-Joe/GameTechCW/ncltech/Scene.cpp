...
std::vector<GameObject*> Scene::ReadGroundList() const {
	std::vector<GameObject*> list;
	
	for (auto& m : m_RootGameObject->GetChildren()) {
		if (m->GetName().find("ground") != string::npos) {
			list.push_back(m);
		}
	}
...
{
	std::sort(m_TransparentNodeList.begin(), m_TransparentNodeList.end(), FrustrumSortingObject::CompareByCameraDistance_Transparent);
